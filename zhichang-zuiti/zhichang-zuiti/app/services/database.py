...
class Database:
    ...
    def create_user(self, open_id: str) -> User:
        ...
        """, (
            ...
            user.quiz_progress,
            json.dumps(user.quiz_answers),
            user.reinforcement_count,
            user.reinforcement_reset_date,
    ...
    def use_reinforcement(self, open_id: str) -> tuple[User, bool]:
        """使用请外援次数，返回 (user, success)"""
        user = self.get_user(open_id)
        if not user:
            user = self.create_user(open_id)
